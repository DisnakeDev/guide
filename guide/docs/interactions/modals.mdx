...
Received when the modal submission encounters an error. It returns the error as well as a <DocsLink reference="disnake.ModalInteraction" /> object that can be used to respond to the user.

````python title="modals.py" {5-7}
class MyModal(disnake.ui.Modal):
    def __init__(self):
        super().__init__(...)

    async def on_error(self, error: Exception, inter: disnake.ModalInteraction):
        await inter.response.send_message(f"An error occured!\n```{error}```")
