...
-   `style`: Utilizes one of <DocsLink reference="disnake.TextInputStyle" /> for the component.
-   `placeholder`: The placeholder text that is shown if nothing is entered.
-   `value`: The pre-filled value of the text input (upto character limit).
...
-   A modal can have a total of **5 components**.
