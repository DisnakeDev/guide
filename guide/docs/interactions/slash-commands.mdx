...
<DiscordMessages>
	<DiscordMessage profile="bot">
		...
		Latency: <DiscordCode>42ms</DiscordCode>!
	</DiscordMessage>
</DiscordMessages>
<br />
...
If you want to disable the automatic registration, use <DocsLink ext="commands" reference="disnake.ext.commands.CommandSyncFlags.none">CommandSyncFlags.none()</DocsLink>.
If you want to see how exactly the list of registered commands changes, set
the <DocsLink ext="commands" reference="disnake.ext.commands.CommandSyncFlags.sync_commands_debug">CommandSyncFlags.sync_commands_debug</DocsLink> flag to `True`.
...
An interaction can only be responded to **once**. After a response is made, **no more responses can be made.** See the [followup](#followups) object for sending messages after responding.
:::

1. <DocsLink reference="disnake.InteractionResponse.send_message">send_message</DocsLink> - Sends response message.
1. <DocsLink reference="disnake.InteractionResponse.edit_message">edit_message</DocsLink> - Edits message, for example
   in component or component+modal interactions. Cannot be used with application commands.
1. <DocsLink reference="disnake.InteractionResponse.defer">defer</DocsLink> - Defers the interaction.
1. <DocsLink reference="disnake.InteractionResponse.send_modal">send_modal</DocsLink> - Sends a
   <DocsLink reference="disnake.ui.Modal">Modal</DocsLink> as a response.
...
- The interaction must have been responded to (see [responding](#responding)).
...
<DiscordMessages>
	<DiscordMessage profile="bot">
		<DiscordCommand slot="reply" profile="user" command="/timer"></DiscordCommand>
		Setting a timer for 30 seconds.
	</DiscordMessage>
	<DiscordMessage profile="bot">
		<DiscordReply slot="reply" profile="bot" command>
			Setting a timer for 30 seconds.
		</DiscordReply>
		<DiscordMention>AbhigyanTrips</DiscordMention>, your timer expired!
	</DiscordMessage>
</DiscordMessages>
}
