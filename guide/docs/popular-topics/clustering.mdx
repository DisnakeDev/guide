...
Personally, over at <a href="https://suggestions.gg/">SuggestionsBot</a> we need to run **1** cluster for every **10** shards the bot needs to connect. So with **53** shards at the time of writing, this means we need to run **6** clusters.

:::note

This number varies wildly, with bots such as <a href="https://gearbot.rocks/">GearBot</a> needing **1** cluster for every **2** shards and purely comes down to how computationally intensive your bot is.

I would start with **1** cluster for every **5** to **10** shards and then modify these numbers depending on how your bot performs.
