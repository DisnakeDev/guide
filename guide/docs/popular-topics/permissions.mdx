...
or <DocsLink reference="disnake.abc.PrivateChannel" />, then the easiest way to query permissions
for a member/role is through <DocsLink reference="disnake.abc.GuildChannel.permissions_for" />.
...
When working with interactions (i.e., with application commands or components), there are two main methods
defined on <DocsLink reference="disnake.Interaction" />: <DocsLink reference="disnake.Interaction.permissions" />
(for getting interaction author's permissions) and <DocsLink reference="disnake.Interaction.app_permissions" />
(for getting bot's permissions in interaction's channel).
