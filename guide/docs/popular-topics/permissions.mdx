...
objects, each permission in them can have three values: `True` (explicitly allowed), `False` (explicitly denied) and
_`None`_ ("untouched", i.e., not overriden). The latter is also the default value for permissions in permission overwrites.
...
# Although not being the recommended usage, you can also access permissions
