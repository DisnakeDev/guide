...
:::warning
It is often misunderstood that setting `True` or `False` on a `Permissions` object will permanently "grant"/"deny"
...
[calculating](#how-permissions-are-calculated) permissions (or [querying](#querying-permissions) a pre-computed ones).
:::
