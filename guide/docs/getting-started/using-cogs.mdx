...
What we code here will extend on the code from [**Initial Files**](./initial-files.mdx), and thus we assume that
...
```python linenums="1" title="ping.py" {10-13}
from disnake.ext import commands
...
```python linenums="1" title="ping.py" {16,17}
from disnake.ext import commands
...
bot.load_extension("cogs.ping")  # Note: We did not mention the .py extension.
...
<sup>Source: <a href="https://docs.disnake.dev/en/latest/ext/commands/cogs.html">Disnake Documentation</a></sup>
