...
[this page](/interactions/slash-commands) for an in-depth coverage of topics, including guild commands, global commands,
options, option types, autocomplete and choices.
...
<DocsLink reference="disnake.User">User</DocsLink> instance in DM contexts, or a<DocsLink reference="disnake.Member">
	Member
</DocsLink> instance in server contexts), which exposes properties such as `.name` or `.id`. (Using just `inter.author`
will give the user's full tag.)
